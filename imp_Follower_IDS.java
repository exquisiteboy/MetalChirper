...
import twitter_source.Follower_IDS;
import util.EnviornmentVariables;

public class imp_Follower_IDS {
	...
	public static Follower_IDS fids;

	private static final DBClass DBClass = null;
	private static DBClass dbobj;
	private static Auth_Class authobj;

	public imp_Follower_IDS(Twitter twitter) {
		try {
			imp_Follower_IDS.Twitter_instance = twitter;
		} catch (Exception ex) {
			imp_Follower_IDS.IS_Exception = true;
			imp_Follower_IDS.Exception_MSG = String.format("Following Error Occured", ex.getMessage());
		}
	}

	public static void getFollowerdIDS(String Screen_name, String Fileoutpt, int max) {
		int maxx = max;
		int countids = 0;
		long cursor = -1;
		boolean braked = true;
		...
		try {
			...
			imp_Follower_IDS.IS_Exception = true;
			imp_Follower_IDS.Exception_MSG = String.format("unable to write file because '%s'", e.getMessage());
		...
		while (braked) {
			...
			fids = new Follower_IDS(imp_Follower_IDS.Twitter_instance);
			fids.getFollowerIDs(Screen_name, cursor);
			do {
				if (!Follower_IDS.IS_Exception) {
					long[] ids = Follower_IDS.followerids;
					...
					if (maxx == -1)
						continue;
					// fids.reset();
				} else if (Follower_IDS.IS_Exception) {
					System.out.println(Exception_MSG);
				}
	...
	public static void getFollowerDir(String filepath, String dirpath, int max) {
		...
		if (Files.isReadable(file) && Files.isWritable(file)) {
			try {
				...
				while ((userName = br.readLine()) != null) {
					String fileOutpath = file.toAbsolutePath() + "/" + userName + "_IDS";
					getFollowerdIDS(userName, fileOutpath, -1);
				}
	...
	public static void main(String[] args) {
		...
		Twitter twitter; dbobj = new DBClass();
		 ...
		 if (database.DBClass.AUTH_OK) { 
			 ...
			 authobj = new Auth_Class(DBClass, EnviornmentVariables.FOLLOWER_LIMIT);
			 if(twitter_source.Auth_Class.IS_OK) { 
				 twitter = authobj.get_twitter_instance();
				 new imp_Follower_IDS(twitter); 
				 imp_Follower_IDS.getFollowerDir("./names", "/home/orbit/eclipse-workspace", -1);
